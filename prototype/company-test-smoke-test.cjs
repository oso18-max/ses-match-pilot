...
assert.match(appSource, /企業テストの合格ライン/);
assert.match(appSource, /点数・除外理由・メール文面の3つ/);
assert.match(appSource, /function companyTestNextDecision/);
assert.match(appSource, /PM判定/);
assert.match(appSource, /function companyTestReadinessItems/);
assert.match(appSource, /渡す前チェック/);
assert.match(appSource, /ダミー\/匿名のみ/);
assert.match(appSource, /function companyTestInviteMessage/);
assert.match(appSource, /案内文コピー/);
...
assert.match(report, /提案候補基準/);
assert.match(report, /判定人材数/);
assert.match(report, /提案候補人材/);
assert.match(report, /候補人材ランキング/);
assert.match(report, /送信可能: 1件/);
assert.match(report, /PM判定/);
assert.match(report, /企業コメント回収|本番連携設計へ進む/);
assert.match(report, /score ok/);
assert.equal(app.companyTestFeedbackStatus().ready, true);
assert.match(report, /確認済み/);
assert.equal(app.companyTestNextDecision({ match, targets: [{ canSend: true }] }).label, "本番連携設計へ進む");
assert.equal(app.companyTestNextDecision({ match: restrictedMatch, targets: [{ canSend: false, blocked: ["年齢NG"] }] }).label, "送信先条件を見直す");
assert.equal(app.companyTestReadinessItems().length, 4);
assert.equal(app.companyTestReadinessItems().some((item) => item.item === "安全"), true);
assert.match(app.companyTestInviteMessage(), /SES Auto Send 企業テストのお願い/);
assert.match(app.companyTestInviteMessage(), /結果レポートを保存して共有する/);
...
assert.equal(typeof app.updateCompanyTestNumber, "function");
assert.equal(typeof app.companyTestFeedbackStatus, "function");
assert.equal(typeof app.companyTestNextDecision, "function");
assert.equal(typeof app.companyTestReadinessItems, "function");
assert.equal(typeof app.companyTestInviteMessage, "function");
assert.equal(typeof app.copyCompanyTestInviteMessage, "function");
...
assert.equal(app.companyTestPackage().minScore, app.state.companyTest.minScore);
assert.equal(app.companyTestPackage().topTalentLimit, app.state.companyTest.topTalentLimit);
...
app.applyCompanyTestPackage({ minScore: 70, topTalentLimit: 2 });
assert.equal(app.state.companyTest.minScore, 70);
assert.equal(app.state.companyTest.topTalentLimit, 2);
