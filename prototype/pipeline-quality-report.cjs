...
function summarize(filePath) {
  ...
  return {
    ...
    customerImported: result.scenario.customers.length,
    customerExcluded: result.scenario.customers.filter((customer) => !customer.sendable).length,
    pending: result.scenario.pending.length,
    sendable: result.sendableRows.length,
    drafts: result.drafts.length,
    draftProposals: result.drafts.length,
