...
const beforeMailReview = app.mailReviewSummary().total;
app.markMailReviewed("req_003", "案件として確認済み");
assert.equal(app.mailReviewSummary().total, beforeMailReview - 1);
assert.equal(app.state.mailReviewHistory[0].action, "案件として確認済み");
